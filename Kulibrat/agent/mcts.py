...
class MCTSAgent(Agent):
    ...
    '''
    def __init__(self, game : Kulibrat, player : Player, c = 1.0, max_sim = 15, score_f = lambda x: x, score_depth = 100000):
        '''
        game : Kulibrat
        ---
        ...
        self.tree_root = MCTS(self.player, state = game.copy_state(), c = c, max_sim = max_sim, score_f = score_f, score_depth=score_depth)
        self.c = c
...
class MCTS():
    ...
    '''
    def __init__(self, player : Player, state : Kulibrat, c = 1.0, max_sim = 15, score_f = lambda x: x, parent=None, parent_action=None, score_depth = 100000):
        self.state = state
        self.player = player
        self.parent = parent
    ...
    def expand(self, action : Action) -> MCTS:
        ...
        child_node = MCTS(self.player, next_state, self.c, self.max_sim, self.score_f, parent=self, parent_action=action, score_depth=self.score_depth)
        self.children[action] = child_node
        return child_node 
    
    
    def is_terminal_node(self, max_score) -> bool:
        if self.state.score[Player.BLACK] >= max_score or self.state.score[Player.RED] >= max_score:
            return True
    ...
    def tree_policy(self) -> MCTS:
        current_node = self
        max_score = max(self.state.score[Player.BLACK], self.state.score[Player.RED]) + self.score_depth
