...
class Coord:
    ...
    def __repr__(self):
        if self.row >= N_ROWS or self.row < 0:
            return "GOAL"
        else:
...
class Pawn:
    ...
    def copy(self):
        return Pawn(
            ...
            self.number,
            Coord(self.position.row, self.position.col) if self.position else None,
        )
...
class Grid:
    ...
    def __init__(self):
        self.grid = {
            Coord(i, j): Pawn(Player.EMPTY, None, None)
...
class Spawn(Action):
    ...
    def __eq__(self, other):
        if type(self) != type(other):
            return False
        return (self.player, self.pawn, self.position) == (
            other.player,
            other.pawn,
            other.position,
        )
    ...
    def __repr__(self):
        return (
            f"Spawn {self.pawn.player.name} PAWN {self.pawn.number} IN {self.position}"
        )
...
class DiagonalMove(Action):
    def __init__(self, player: Player, pawn: Pawn, direction):
        ...
        super().__init__(player, pawn)
        self.dest = Coord(
            pawn.position.row + player.row_dir(), pawn.position.col + direction
        )

    def __eq__(self, other):
        if type(self) != type(other):
            return False
        return (self.player, self.pawn, self.dest) == (
            other.player,
            other.pawn,
            other.dest,
        )
...
class Kulibrat(object):
    ...
    def __init__(self, max_score: int = 5):
        ...
        self.pawns = {
            Player.BLACK: [Pawn(Player.BLACK, i) for i in range(N_PAWNS)],
            Player.RED: [Pawn(Player.RED, i) for i in range(N_PAWNS)],
        }
    ...
    def copy_state(self) -> Kulibrat:
        ...
        new.pawns = {
            Player.BLACK: [pawn.copy() for pawn in self.pawns[Player.BLACK]],
            Player.RED: [pawn.copy() for pawn in self.pawns[Player.RED]],
        }
    ...
    def __eq__(self, other) -> bool:
        if type(self) != type(other):
            return False
        return (self.grid, self.turn, self.score, self.pawns, self.max_score) == (
            other.player,
            other.turn,
            other.score,
            other.pawns,
    ...
    def pre_turn(self, action: Action):
        ...
        if action not in self.allowed_actions:
            raise ValueError(
                f"Forbidden move {str(action)}, allowed actions {self.get_possible_actions()}"
            )
    ...
    def move_pawn(self, pawn_: Pawn, dest: Coord):
        ...
        if (
            pawn_.player is not None
            and pawn_.player != Player.EMPTY
            and pawn_.number is not None
        ):
    ...
    def get_possible_actions(self) -> List[Action]:
        ...
        spawn_pawn_n = min(
            (
                ...
                if self.get_pawn_by_id(self.turn, i).position is None
            ),
            default=None,
        )
        if spawn_pawn_n is not None:  # If all pawns are not used
            spawn_pawn = self.get_pawn_by_id(
                self.turn, spawn_pawn_n
            )  # Pawn to spawn (the unused one with the lowest id)
            for col in range(N_COLS):
        ...
        for pawn in self.get_pawns_by_player(self.turn):
            if pawn.is_placed():
                for col_move_dir in [EAST, WEST]:  # -1 and 1
                    ...
                    if not (
                        self.check_valid_coord(dest) or self.turn.check_goal_coord(dest)
                    ):
                        continue
